...
ens_g_names="ens_id_as1_w_G_names_locs.txt" #created with this command: cat Gac_white_70hpf_ens.bed | cut -f 4 | grep "ENS" > ens_id_w_G_name.txt
new="Gac_white_70hpf_ens_as_1_modnames_for_cells.bed"
...
with open(gtf_name, "r") as gtf, open(new, "w") as newby:
    for line in gtf:
        ...
        #then proceed
        #if it is on the mitochondrial chromosome, then append MT to the ensembl id
        #finally replace the gene id with the ensembl id 
        ...
        if gname not in genes_w_multiple_ens_ids_for_one_g_id and gname in geneids:
            ...
            if line[0]=="MT":
                ...
                newname=myinfo[5]
                        #print(name)
            line[3]=line[3].replace(gname, newname)
            #print(line)
            s = "\t".join(line)
            newby.write(s + "\n")
            #print("done")
        ...
        #we then repeated the MT renaming and then writing out to the new file as done in the previous if statement
            ...
            #finding the gene that this record is the best match 
             bestmatch=''
             bestmatchdist=0
             bestmatchdist2=0
            ...
            # print("FINAL PICK", bestmatcht, bestmatchdist)
             ...
             if line[0]=="MT":
                 bestmatchg="MT"+bestmatchg
             else:
                 bestmatchg=bestmatchg
            #print(name)
             line[3]=line[3].replace(gname, bestmatchg)
        ...
        #then the line will stay the same with the exception of the genes on the mitochondrial chromosome
            ...
            line[3]=line[3].replace(gname, name)
            s = "\t".join(line)
            c+=1
            newby.write(s + "\n")
